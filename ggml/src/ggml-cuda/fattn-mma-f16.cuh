...
#pragma unroll
    for (int col = 0; col < cols_per_thread; ++col) {
        ...
        // all inputs are finite. Gated on rel_f so only the banded op (whose V is that large) pays
        // for it. When the per-thread partial row sum of weights gets large, force
