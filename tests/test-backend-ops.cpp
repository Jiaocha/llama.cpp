    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            if (strcmp(t->name, "a") == 0) {
                ...
                const float scale = 1.0f / sqrtf((float) n_cols);

                auto is_pow2 = [](const int64_t x) {
                    return x > 0 && (x & (x - 1)) == 0;
                };

                const bool use_h12 = n_cols % 12 == 0 && is_pow2(n_cols / 12);
                const bool use_h20 = n_cols % 20 == 0 && is_pow2(n_cols / 20);

                if (use_h12 || use_h20) {
                    const int64_t B = use_h12 ? 12 : 20;
                    ...
                    for (int64_t r = 0; r < n_rows; ++r) {
                        ...
                        for (int64_t i = 0; i < n_cols; ++i) {
                            ...
                            const float sign_pow2 = (pop % 2 == 0) ? 1.0f : -1.0f;
                            const float sign_base = B == 12 ? H12[c_m][r_m] : H20[c_m][r_m];

                            row_data[i] = scale * sign_base * sign_pow2;
                        }
                    }
                } else {
                    for (int64_t r = 0; r < n_rows; ++r) {
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_mul_mat_hadamard(GGML_TYPE_F32, GGML_TYPE_F32, 384, 4, 384, {2, 3}));
