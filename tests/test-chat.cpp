...
int main(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--template") {
            ...
            positional.push_back(arg); // .jinja template path
        }
    }
    ...
    // keep the detailed output unless the user asked for a lower verbosity threshold
    if (detailed_debug) {
        common_log_set_verbosity_thold(params.verbosity < LOG_DEFAULT_LLAMA ? params.verbosity : 999);
    }
    ...
    for (const auto & path : positional) {
        if (path.size() >= 6 && path.rfind(".jinja") == path.size() - 6) {
