...
A KV-depth bucket gets a row only for a config that is at least 2% faster in aggregate over the launches of 1024 tiles or more and has no clear loss (more than 1.5%) at any large or partial-tile launch, so a device where the wide tile does not pay off emits nothing and stays at baseline.
The last number of a row is `tiles_min`: the smallest sampled small launch of full tiles in that KV-depth bucket from which the config wins at every larger sampled small launch. Below it the baseline tile is kept.
A win must clear 1.5%. A cell whose first measurement is closer to baseline than that is measured two more times, and the decision uses the median ratio over the three. A cell still that close after the retries is not a win, so at a small launch it moves `tiles_min` up; at a large or partial-tile launch it does not veto the config, its time simply stays in the aggregate that the 2% gate is applied to.
